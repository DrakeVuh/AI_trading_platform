...
def _fetch_via_ccxt(
    ...
    while cursor < until_ms:
        ...
        try:
            batch = exchange.fetchFundingRateHistory(symbol, since=cursor, limit=limit, params=params)  # type: ignore[attr-defined]
        except Exception:
            return []
...
def _symbol_directory(data_root: Path, symbol: str) -> Path:
...
def _save_partitions(symbol_dir: Path, df: pd.DataFrame, symbol: str, bucket: str) -> None:
    if df.empty:
        return

    df = df.copy()
    df["month_key"] = df["timestamp"].dt.tz_convert("UTC").dt.strftime("%Y-%m")

    for month_str, group in df.groupby("month_key"):
        path = parquet_path(symbol_dir, month_str)

        start = time.perf_counter()
        if path.exists():
            existing = pd.read_parquet(path)
            combined = pd.concat([existing, group.drop(columns="month_key")], ignore_index=True)
        else:
            combined = group.drop(columns="month_key")
...
def collect_funding_rates(
    ...
    if use_ccxt:
        entries = _fetch_via_ccxt(symbol, since_ms, until_ms, bucket)

    if not entries:
        entries = _fetch_via_http(symbol, since_ms, until_ms, bucket)
    ...
    symbol_dir = _symbol_directory(data_root, symbol)
    _save_partitions(symbol_dir, df, symbol, bucket)
...
def main() -> None:
    ...
    parser.add_argument("--until", required=True, type=_parse_datetime, help="End datetime (ISO, UTC, exclusive)")
    parser.add_argument("--data-root", default=str(DEFAULT_BASE_DIR), help="Base data directory")
    parser.add_argument(
